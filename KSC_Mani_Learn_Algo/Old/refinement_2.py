...
# If ever use the fake way, use more useless inputs to function here:
def create_sc_sigma_tilde(A, n, k):
...
###############################################################################
###############################################################################
# Refine: There could be ties here! 

def test(data1, data2, sigma_tilde_final, K):
    D1 = np.zeros((n,n))
    D3 = np.zeros((n,n))
    for i in range(0,n):
...
def refine(X, n, k, sigma, perms, p, rho, nb_size_manifolds, nb_size_X, K, sigma_tilde):

    # Compare the various methods:
    coeff_0 = np.repeat(1, n**2).reshape(n,n)
    error_array = np.empty([3, 3])

    mani_est_0 = estimate_manifold(X, sigma_tilde, n, k, p, np.repeat(1,k), coeff_0)
    mani_est_1 = estimate_manifold(X, sigma_tilde, n, k, p, nb_size_manifolds, coeff_0)
    mani_est_2 = estimate_manifold(X, sigma_tilde, n, k, p, nb_size_manifolds, A)
    X_new_0 = create_X_new(X, n, p, ((1)), coeff_0)
    X_new_1 = create_X_new(X, n, p, nb_size_X, coeff_0)
    X_new_2 = create_X_new(X, n, p, nb_size_X, A)
    ...
    for j in range(len(list_mani_est)):
        for l in range(len(list_X_new)):
            ...
            X_new = list_X_new[l]
            sigma_hat = test(X_new, mani_est, sigma_tilde, K)
            sigma_hat_final = perm_true(sigma, sigma_hat, n, k, perms)
            error_array[j,l] = np.sum(sigma_hat_final != sigma)
    ...
    # Reformat:
    number_outputs = 10 # FIX
    errors = error_array.reshape(number_outputs-1,)
    errors_final = np.append(error_init, errors)
    return (100*(errors_final/n))


# Another version, this time with dimension reduction:
def refine_dr(X, n, k, sigma, perms, p, rho, nb_size_manifolds, nb_size_X, K, sigma_tilde):

    X_dr = reduce_dimension(X, sigma_tilde, n, p, k, nc)
    centers = estimate_manifold_kmeans(X_dr, sigma_tilde, sigma_centers, n, p, k, nc)
    sigma_hat = test_dr(X_dr, centers, sigma_centers, n, nc, k, K)
    ...
    error_init = np.sum(sigma_tilde_final != sigma)
    error = np.sum(sigma_hat_final != sigma)

    #
    return error_init, error
...
################################################################################
# Manifold lists:
number_outputs = 10 # length of each vector output from algorithm() function
...
print refine_dr(X, n, k, sigma, perms, p, rho, nb_size_manifolds, nb_size_X, K, sigma_tilde)
